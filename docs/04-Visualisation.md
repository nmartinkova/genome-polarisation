...
# Visualisation of polarised genotypes {#visualisation}
...
If the input data were generated from a sorted VCF file (Chapter \@ref(vcf2diem)), marker positions are already available.
...
If the default marker axis needs adjustments, such as here, where accession numbers overlap, use `plotMarkerAxis` directly. This returns an `axisInfo` list so you can modify labels before redrawing. `plotMarkerAxis` also accepts additional graphical arguments. For the full list of supported graphical arguments, check the function help with `?plotMarkerAxis`.
...
The `axisInfo` list will now contain five elements with plotting data. Let's shorten the contig names and convert the tick labels to base pairs (default is to display millions of base pairs). This list can then be used for plotting directly, without the need to recalculate the position of the axis elements from the `*-includedSites.txt` file.
